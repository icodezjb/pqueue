...
func TestSStack(t *testing.T) {
	...
	stack := newSstack()
	for rep := 0; rep < 2; rep++ {
		// Push all the data into stack, Pop out every even index
		even := []*item{}
		for i := range data {
			stack.Push(data[i])
			if i%2 == 0 {
				even = append(even, stack.Pop().(*item))
			}
		}
		...
		// Make sure the contents of resulting slices are ok
		for i := range data {
			if i%2 == 0 && data[i] != even[i/2] {
				t.Errorf("push/pop mismatch: have %v, want %v.", even[i/2], data[i])
			}
...
func TestSStack_Sort(t *testing.T) {
	// Create some initial data
	size := 16 * blockSize
	data := make([]*item, size)
	for i := 0; i < size; i++ {
...
func TestSStack_Reset(t *testing.T) {
	// Create some initial data
	size := 16 * blockSize
	data := make([]*item, size)
	for i := 0; i < size; i++ {
